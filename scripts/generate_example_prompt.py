...
def generate_example_prompt() -> str:
    ...
    tool_guidance = _format_available_tool_guidance(available_tool_names=available_tool_names)
    names_block = "\n".join(sorted(available_tool_names))
    schema_block = _format_tool_schema_summary(tools)
    required_search_block = build_user_prompt(
        user_name="jacen",
        user_id=266241639003979776,
        user_global_name="jacen",
        owner_context="The current user is the configured bot owner/admin.",
        current_datetime_text="Current local date/time for the user: 2026-06-01 11:42 America/Los_Angeles",
    ...
    search_section = _extract_required_search_section(required_search_block)
    return (
        ...
        "context message, two appended tool-policy `user` messages, and a separate native `tools` schema array.\n\n"
        "## Message 1: system\n\n"
        f"```text\n{get_system_prompt()}\n```\n\n"
        ...
        "```\n\n"
        "## Native tools array\n\n"
        "The provider request also includes native OpenAI-compatible function schemas for all chat tools. "
...
def _format_available_tool_guidance(*, available_tool_names: set[str]) -> str:
    action_tools = sorted(
    ...
    guidance = (
        ...
        f"- {', '.join(sorted(available_tool_names)) if available_tool_names else '(none)'}\n"
...
def _extract_required_search_section(prompt: str) -> str:
    marker = "Required tool use for this request:"
    start = prompt.find(marker)
    if start < 0:
        return f"- The user included `use search`, so you must call `{WEB_SEARCH_TOOL_NAME}` at least once."
    end = prompt.find("\n\n", start)
