...
class ActiveRequestRegistry:
    def __init__(self) -> None:
        self._tasks: dict[RequestKey, asyncio.Task[str]] = {}

    def has_active(self, key: RequestKey) -> bool:
        task = self._tasks.get(key)
        return task is not None and not task.done()

    def start(self, key: RequestKey, coro: Coroutine[Any, Any, str]) -> asyncio.Task[str]:
    ...
    def clear(self, key: RequestKey, task: asyncio.Task[str]) -> None:
