...
def format_help_message(page: int = 1) -> str:
    pages = {
        1: (
            "**Nycti Help 1/2**\n"
            "Triggers:\n"
            "- mention the bot or reply to a bot message (default)\n"
            "- a leading configured name or ambient questions in approved channels may also be enabled\n\n"
            "Core commands:\n"
            ...
            "- `/benchmark suite|failures|trace`: run manifest-based real-LLM evals or inspect saved failures (`Manage Server`)\n"
            "- `/cancel`: cancel your active prompt in this channel\n"
            "- `/confirm proposal:<id>`: confirm one exact pending action\n"
        ...
        2: (
            ...
            "- `/emoji`: list/info, alias overrides, manual import (`/confirm`), pin/block controls, meaning overrides, and backfill\n"
